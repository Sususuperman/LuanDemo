...
import com.hywy.luanhzt.adapter.menu.MenuParentAdapter;
import com.hywy.luanhzt.adapter.menu.MyAdapter;
import com.hywy.luanhzt.app.App;
import com.hywy.luanhzt.drag.DragCallback;
...
//				.backicon(R.drawable.ic_arrow_back_black_24dp)
//				.title(getString(R.string.approval_result2)));
...
//        init(indexDataList);
        List<MenuEntity> indexDataList = (List<MenuEntity>) appContext.readObject(AppConfig.KEY_All);
        List<MenuEntity> mainList = (List<MenuEntity>) appContext.readObject(AppConfig.KEY_USER);
        //获取首页未展示的项目列表
        indexDataList.removeAll(mainList);
        Log.i("list", indexDataList.size() + "");
        init(indexDataList);
    }
...
//                    menuList.get(i).getChilds().get(k).setSelect(true);
                    menuList.get(i).getChilds().remove(menuEntity);
                }
            }
        }
        menuParentAdapter.notifyDataSetChanged();
